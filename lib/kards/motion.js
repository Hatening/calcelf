...
module.exports = {
  ...
  extract(problem, opts = {}) {
    ...
    // —— 环形/闭合跑道：直线渲染器无法正确表达，交给带"closed-loop 拓扑规律"的 free 路径 ——
    const CIRCULAR_RE = /环形|環形|圆形跑道|circular track|running track|closed loop|around the track|oval track|円形|周回|원형|트랙|piste circulaire|anneau|kreisbahn|pista circolare|pista circular|مسار دائري|المضمار|حلقة|مسیر دایره|حلقه/;
    if (CIRCULAR_RE.test(tl)) return null;
